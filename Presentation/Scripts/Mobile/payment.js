        ...
        try {
            ...
            switch (_tabs) {
                ...
                case this.Tabs.finalSubmit:
                    ...
                    //else {
                    //    $('.con_booking span').addClass('progressBar');
                    //    tracking.populateLeadPayment($('#BillingDetails_CCHolderName').val());
                    //}
                    break;
            }
...
$(".flt_dtl_btn a").click(function () {if ($(this).text() === "+ Show Flight Details") {$(this).text("- Hide Flight Details");}
    else {$(this).text("+ Show Flight Details");}
    $(".itnr_main_wrapper").slideToggle("slow");
});
...
function cardFormValidate() {
    ...
    $('#BillingDetails_CardNumber').validateCreditCard(function (result) {
        ...
        if (result.valid && result.length_valid && result.luhn_valid && cardType != "") {
            ...
            if (cardType == 'visa') {
                var backPosition = result.valid ? '2px -38px, 260px -87px' : '2px -163px, 260px -61px';
                var selectVale = 1;
            } else if (cardType == 'visa_electron') {
                var backPosition = result.valid ? '2px -225px, 260px -87px' : '2px -163px, 260px -61px';
                var selectVale = 6;
            } else if (cardType == 'mastercard') {
                var backPosition = result.valid ? '2px -263px, 260px -87px' : '2px -247px, 260px -61px';
                var selectVale = 2;
            } else if (cardType == 'maestro') {
                var backPosition = result.valid ? '2px -263px, 260px -87px' : '2px -289px, 260px -61px';
                var selectVale = 7;
            } else if (cardType == 'discover') {
                var backPosition = result.valid ? '2px -187px, 260px -87px' : '2px -331px, 260px -61px';
                var selectVale = 5;
            } else if (cardType == 'amex') {
                var backPosition = result.valid ? '2px -113px, 260px -87px' : '2px -121px, 260px -61px';
                var selectVale = 3;
            } else if (cardType == 'jcb') {
                var backPosition = result.valid ? '2px -338px, 260px -87px' : '2px -121px, 260px -61px';
                var selectVale = 9;
            } else if (cardType == 'diners_club_carte_blanche' || cardType == 'diners_club_international') {
                var backPosition = result.valid ? '2px -150px, 260px -87px' : '2px -121px, 260px -61px';
                var selectVale = 4;
            } else {
                var backPosition = result.valid ? '0 -0, 260px -87px' : '2px -121px, 260px -61px';
                var selectVale = 0;
            }
...
$("body").delegate(".brb-redio-btn", "click", function () {
...
$(".btn_sld").click(function () {
    $(".overw").slideToggle("slow");
...
$('.popupopencont').click(function () {
    var dataId = $(this).attr("data-id");
    $('#privacy').modal('show');
    try {
        if (dataId === "privacy") {
            popuptitle = "Privacy Policy";
            popupulr = "home/popupprivacypolicy/";
        }
        if (dataId === "tandc") {
            popuptitle = "Terms and Conditions";
            popupulr = "home/popuptermsandconditions/";
        }
        if (dataId === "payacpt") {
            popuptitle = "Payment Acceptance";
            popupulr = "home/popuppaymentacceptance/" + $('#guid').val();
        }
        if (dataId === "TravelInsPop") {
            popuptitle = "Travel Insurance";
            popupulr = "home/TravelInsurancePopup/";
        }
        if (dataId === "TrvlInsOverPop") {
            popuptitle = "Travel Insurance Overview";
            popupulr = "home/TravelInsuranceOverviewPopup/";
        }
        $('.pouptitle').html(popuptitle);
        $.ajax({
            url: DOMAIN_URL + popupulr,
            type: 'GET',
            contentType: "application/json; charset=utf-8",
            success: function (response) {
                if (response.IsSuccess) {
                    $("#mobilepopup").html(response.HtmlResponse);
                }
                $('.page-heading').hide();
                $('.pay-mob-mod ul').css("list-style-position", "inside");
